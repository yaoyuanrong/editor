<template>
  <node-view-wrapper class="umo-node-view">
    <t-popup :attach="`${container} .umo-zoomable-container`" overlay-inner-class-name="umo-editor-bubble-menu"
      trigger="click" :visible="editor?.isEditable &&
        bubbleMenu &&
        editor.state.selection.to === editor.state.selection.from
        " @visible-change="(visible: boolean) => (bubbleMenu = visible)">
      <div class="umo-node-container hover-shadow umo-node-callout" :style="{
      ...
      <template #content>
        ...
        <menus-bubble-callout-emoji-remove v-if="editor.getAttributes('callout').icon" />
